...
var aliyunHTTPClient = &http.Client{
    Transport: newPacedTransport(&http.Transport{
        // Honour HTTPS_PROXY so captcha verification egresses from the same
        // IP that harvested the device tokens — Aliyun risk control rejects
        // verification when the request IP mismatches the token's origin.
...
func dialUTLS(ctx context.Context, network, addr string) (net.Conn, error) {
    ...
    // uTLS config — advertise the real Chrome ALPN list (h2 first).
    // Some Aliyun ESA nodes (e.g. 8.216.x international ALB) FORCE h2 for
    // Chrome-fingerprinted ClientHellos regardless of ALPN, which breaks an
    // http/1.1-only announcement ("malformed HTTP response" h2 frames).
    // Advertising [h2, http/1.1] lets the server pick, and utlsConnAdapter
    // surfaces the negotiated protocol so net/http routes h2 conns through
    // its bundled HTTP/2 implementation (ForceAttemptHTTP2).
    config := &utls.Config{
        ServerName:         host,
...
// crypto/tls.ConnectionState surface net/http checks when deciding whether a
// custom-DialTLS connection speaks HTTP/2. Without it, Transport cannot see
// the negotiated protocol and would pipeline HTTP/1.1 into an h2-only
// connection ("HTTP/1.x transport connection broken").
...
var zaiHTTPClient = &http.Client{
    Transport: newPacedTransport(&http.Transport{
        ...
        ExpectContinueTimeout: 1 * time.Second,
        // Must be true: with a custom DialTLSContext the bundled HTTP/2 is
        // disabled by default, which silently downgrades conns the server
        // negotiated as h2 (some Aliyun ESA nodes FORCE h2 for Chrome
        // fingerprints) into broken HTTP/1.1 reads.
        ForceAttemptHTTP2:     true,
    }),
    Jar: zaiJar,
}
